describe("Integration :: generatePossibilities() Test", function() {
	describe("No exclusions and musthaves", function() {
		it("One question mark, one possibility", function() {
			chai.assert.sameMembers(generatePossibilities("?=1")[PRIORITY_STANDARD], ["1=1"])
		})
		it("Add to ten", function() {
			var res = generatePossibilities("?+?=10")
			chai.assert.sameMembers(res[PRIORITY_OPENING], ['2+8=10', '3+7=10', '4+6=10', '6+4=10', '7+3=10', '8+2=10'])
			chai.assert.sameMembers(res[PRIORITY_STANDARD], ['1+9=10', '5+5=10', '9+1=10'])
			chai.assert.isEmpty(res[PRIORITY_WEIRD])
		})
		it("Stripping of Excess Zeros", function() {
			chai.assert.sameMembers(generatePossibilities("00?*?=0?")[PRIORITY_WEIRD], ['000*0=00', '000*1=00', '000*2=00', '000*3=00', '000*4=00', '000*5=00', '000*6=00', '000*7=00', '000*8=00', '000*9=00', '001*0=00', '001*1=01', '001*2=02', '001*3=03', '001*4=04', '001*5=05', '001*6=06', '001*7=07', '001*8=08', '001*9=09', '002*0=00', '002*1=02', '002*2=04', '002*3=06', '002*4=08', '003*0=00', '003*1=03', '003*2=06', '003*3=09', '004*0=00', '004*1=04', '004*2=08', '005*0=00', '005*1=05', '006*0=00', '006*1=06', '007*0=00', '007*1=07', '008*0=00', '008*1=08', '009*0=00', '009*1=09'])
		})
	})
	
	describe("Exclusions only", function() {
		it("Impossible exclusions provide empty result", function() {
			var res = generatePossibilities("?+?=3", "3")
			chai.assert.isEmpty(res[PRIORITY_OPENING])
			chai.assert.isEmpty(res[PRIORITY_STANDARD])
			chai.assert.isEmpty(res[PRIORITY_WEIRD])
		})
		it("Add to ten, with exclusions", function() {
			chai.assert.sameMembers(generatePossibilities("?+?=10", "234")[PRIORITY_STANDARD], ['5+5=10', '1+9=10', '9+1=10'])
		})
		it("Add to ten, but exclusions imply addition", function() {
			var res = generatePossibilities("???=10", "234-*/")
			chai.assert.sameMembers(res[PRIORITY_STANDARD], ['1+9=10', '5+5=10', '9+1=10'])
			chai.assert.sameMembers(res[PRIORITY_WEIRD], ['010=10', '+10=10'])
		})
	})
	
	describe("Musthaves only", function() {
		it("Impossible musthaves provide empty result", function() {
			var res = generatePossibilities("?+?=3", "", "7")
			chai.assert.isEmpty(res[PRIORITY_OPENING])
			chai.assert.isEmpty(res[PRIORITY_STANDARD])
			chai.assert.isEmpty(res[PRIORITY_WEIRD])
		})
		it("Add to ten, forcing digits", function() {
			chai.assert.sameMembers(generatePossibilities("?+?=10", "", "12")[PRIORITY_OPENING], ["2+8=10", "8+2=10"])
		})
...
describe("Integration :: Speed Tests", function() {
	it("4 question marks, 3 exclusions, 1 musthaves", function() {
		chai.assert.sameMembers(generatePossibilities("9??=??", "+-0", "2")[PRIORITY_OPENING], ['9*2=18', '9*3=27', '9*8=72'])
	})
	it("5 question marks, no exclusions, 1 musthaves", function() {
		var res = generatePossibilities("?????", "", "=")
		chai.assert.equal(res[PRIORITY_OPENING].length + res[PRIORITY_STANDARD].length + res[PRIORITY_WEIRD].length, 337)
	})
	it("5 question marks, 4 exclusions, 2 musthaves", function() {
		var res = generatePossibilities("?????", "0123", "+=")
		chai.assert.sameMembers(res[PRIORITY_OPENING], ['4+5=9', '5+4=9'])
		chai.assert.sameMembers(res[PRIORITY_STANDARD], ['4+4=8'])
	})
})
